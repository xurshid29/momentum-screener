...
class TickFeedService {
  ...
  private momoSetup = new MomoSetupTracker();

  private flushMomoSetupTransitions(): void {
    for (const transition of this.momoSetup.drainTransitions()) {
      poller.onMomoSetupTransition(transition);
    }
  }
  // ⤴ second variant (2026-08-07): 3/15/8 on 2-minute buckets — the
  // operator's other TV setup. Bars aggregate below from the per-second
  // stream (no tracker exists on this grid otherwise) and persist to
  // bars_2m so the 23-bar warmup survives deploys.
  private macdCurl2m = new MacdCurlTracker(MACD_CURL_2M);
  // Open 2m bucket per known runner: {bucket start, running close, vol}.
  private bucket2m = new Map<string, { bucket: number; open: number; high: number; low: number; close: number; vol: number }>();
  private bar2Buffer: {
  ...
  // ⤴ swing-end variants (2026-08-09): 3/15/8 on the 1h and 4h grids, fed
  // by those reclaim layers' bar streams + seed replays. 4h buckets arrive
  // pre-anchored to the ET session grid by the EMA layer's bucketing.
  private macdCurl1h = new MacdCurlTracker(MACD_CURL_1H);
  private macdCurl4h = new MacdCurlTracker(MACD_CURL_4H);
  ...
  status() {
    return {
      ...
      macd_curl_1h_tracked: this.macdCurl1h.symbolsTracked(),
      macd_curl_4h_tracked: this.macdCurl4h.symbolsTracked(),
  ...
  // Variant → tracker map for the HTF-fed lanes (15m/1h/4h ride the
  // reclaim layers' streams; 5m/2m have their own aggregation).
  private macdForTf(tf: string): MacdCurlTracker | null {
    return tf === '15m' ? this.macdCurl15m : tf === '1h' ? this.macdCurl1h : tf === '4h' ? this.macdCurl4h : null;
  }

  macdSnapshot(ticker: string, livePrice?: number, variant: '5m' | '2m' | '15m' | '1h' | '4h' = '5m'):
    (NonNullable<ReturnType<MacdCurlTracker['snapshot']>> & { chg_pct: number | null; current_price: number }) | null {
    ...
    const tracker = variant === '2m' ? this.macdCurl2m : this.macdForTf(variant) ?? this.macdCurl;
  ...
  private async seedTrackerFromTable(
    ...
    tracker: EmaCrossTracker,
    label: string,
    also?: (ticker: string, closeTs: number, close: number, volume: number) => void,
  ): Promise<void> {
    try {
      const rows = await getDb()
        .selectFrom(table)
      ...
      const flush = () => {
        ...
        for (const b of adjustSplitHistory(buf)) {
          ...
          also?.(curTk, b.closeTs, b.close, b.volume);
        }
        syms++;
        buf = [];
      };
  ...
  private async scanSparse5m(stats: Map<string, { n: number; n24: number; latestMs: number }>): Promise<void> {
    ...
    for (const { tk, n } of batch) {
      ...
      if (this.emaCross.canSeed(tk)) {
        ...
        this.macdCurl.reseedFromHistory(tk, adj);
        reseeded++;
    ...
    for (let i = 0; i < targets.length; i += CHUNK) {
      ...
      for (const tk of chunk) {
        ...
        if (l.tracker.canSeed(tk)) {
          ...
          this.macdForTf(l.cfg.tf)?.reseedFromHistory(tk, adj);
          seeded++;
  ...
  private applyBackfillBars(tk: string, bars: Array<{ closeTs: number; close: number; volume: number }>, consolidated = false): { seeded: number; persisted: number } {
    ...
    if (this.emaCross.canSeed(tk)) {
      ...
      this.macdCurl.reseedFromHistory(tk, recentFirst);
      seeded = 1;
  ...
  private flushBars(): void {
    ...
    if (this.bar2Buffer.length > 0) {
      const rows = this.bar2Buffer.splice(0);
      void getDb()
        .insertInto('bars_2m')
        .values(rows)
        .onConflict((oc) => oc.columns(['ticker', 'bar_ts']).doNothing())
        .execute()
        .catch(() => { /* same error-throttle class as above */ });
    }
    for (const l of this.htfLayers) {
  ...
  private onBar(line: string): void {
    ...
    if (this.technicalEnabled && poller.isKnownRunner(m.s)) {
      ...
      // ⤴ 2m MACD variant: no tracker aggregates on this grid, so bucket the
